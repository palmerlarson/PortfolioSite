...
})({"2P9Ry":[function(require,module,exports) {
"use strict";
...
module.bundle.HMR_BUNDLE_ID = "5d45a21d6322a68e";
...
const init = ()=>{
    ...
    if (window.matchMedia("(prefers-color-scheme: dark)").matches) themeChange();
    //if user swithces to dark mode via personal desktop
    window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change", (event)=>{
        if (event.matches) themeChange();
    });
};
window.onload = init;
const themeChange = ()=>{
    if (document.body.classList.contains("dark")) lightMode();
    else darkMode();
};
const lightMode = ()=>{
    ...
    cdn.href = ``;
    for(let i = 0; i < light.length; i++)light[i].classList.remove("light");
    for(let i1 = 0; i1 < dark.length; i1++)dark[i1].classList.remove("darkDiv");
};
const darkMode = ()=>{
    let light = document.querySelectorAll("a, span");
    let dark = document.querySelectorAll(".card");
...
},{}]},["2P9Ry","ahc7M"], "ahc7M", "parcelRequire5242")

//# sourceMappingURL=index.6322a68e.js.map
